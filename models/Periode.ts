...
interface PeriodeModel extends Model<IPeriode, {}, IPeriodeMethods> {
    getAll(page: number, limit: number): Promise<HydratedDocument<IPeriode, IPeriodeMethods>>;
}
...
PeriodeSchema.method('cascadeDelete', async function cascadeDelete() {
    const  visi_id = await mongoose.model('Visi').find({ periode: this._id });
...
PeriodeSchema.static('getAll', async function getAll(page: number = 1, limit: number = 10) {
