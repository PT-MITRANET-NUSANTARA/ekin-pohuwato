...
const rktSchema = Joi.object({
  ...
  input: Joi.object({
    ...
    target_capaian: Joi.string().required().label("Target Capaian Input"),
    satuan: Joi.string().required().label("Satuan Input"),
  })
    .required()
    .label("Data Input"),
  output: Joi.object({
    name: Joi.string().required().label("Nama Output"),
    target_capaian: Joi.string().required().label("Target Capaian Output"),
    satuan: Joi.string().required().label("Satuan Output"),
  })
    .required()
    .label("Output"),
  outcome: Joi.object({
    name: Joi.string().required().label("Nama Outcome"),
    target_capaian: Joi.string().required().label("Target Capaian Outcome"),
  ...
  "string.length":
    "{{#label}} harus memiliki panjang tepat {{#limit}} karakter.",
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Failed to fetch RKT data" },
      { status: 500 }
    );
  }
}
