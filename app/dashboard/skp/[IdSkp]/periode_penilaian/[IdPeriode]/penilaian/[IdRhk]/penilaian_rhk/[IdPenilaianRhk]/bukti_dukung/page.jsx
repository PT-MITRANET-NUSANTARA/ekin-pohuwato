...
import { ExclamationCircleFilled, ExclamationCircleOutlined, FileOutlined, LinkOutlined, UserOutlined } from '@ant-design/icons';
                        ...
                        <div className="flex flex-col gap-y-4 mb-6">
                            ...
                            <div className="grid grid-flow-row divide-y text-xs">
                                <div className="flex items-center justify-between py-2">
                                    <span className="uppercase font-semibold">Deskripsi</span>
                                    {rhks?.desc}
                                </div>
                                <div className="flex items-center justify-between py-2">
                                    <span className="uppercase font-semibold">Jenis</span>
                                    {rhks?.jenis}
                                </div>
                                <div className="flex items-center justify-between py-2">
                                    <span className="uppercase font-semibold">Klasifikasi</span>
                                    {rhks?.klasifikasi}
                                </div>
                    ...
                    <div className="grid grid-cols-12 gap-4">
                        <div className="p-2 rounded-md col-span-3 bg-white h-fit">
                            {menu && menu.length > 0 ? (
                                ...
                                <div className="flex items-center gap-x-4 p-3  text-blue-600 rounded-md">
                                    <ExclamationCircleFilled className="text-blue-500 text-lg" />
                                    <span>Item Bukti Dukung Kosong</span>
                                </div>
                            )}
                        </div>
                        <Card className="col-span-3 h-fit">
                            {menuRHK && menuRHK.length > 0 ? (
                                <Menu className="col-span-3" mode="inline" items={menuRHK} selectedKeys={[activeRHK]} />
                            ) : (
                                <div className="flex items-center gap-x-4 p-3  text-blue-600 rounded-md">
                                    <ExclamationCircleFilled className="text-blue-500 text-lg" />
                                    <span>Pilih Item Bukti Dukung Terlebih Dahulu</span>
                                </div>
                            )}
